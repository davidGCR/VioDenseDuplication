def train(_loader, _epoch, _model, _criterion, _optimizer, _device, _config, _accuracy_fn, _verbose=False):
    ...
    for i, data in enumerate(_loader):
        ...
        #accuracy
        # acc = _accuracy_fn(labels, outs)
        pred = np.round(outs.detach().cpu().numpy())
    ...
    train_acc = _accuracy_fn(y_true,y_pred)
    print(
        'Epoch: [{}]\t'
        'Loss(train): {loss:.4f}\t'
...
def val(_loader, _epoch, _model, _criterion, _device, _config, _accuracy_fn):
    ...
    for _, data in enumerate(_loader):
        ...
        with torch.no_grad():
            ...
            pred = np.round(outputs.detach().cpu().numpy())
            target = labels.cpu().numpy().tolist()
            y_true.extend(target)
            y_pred.extend(pred.reshape(-1).tolist())
    ...
    val_loss = losses.avg
    val_acc = _accuracy_fn(y_true,y_pred)
    print(
        'Epoch: [{}]\t'
        'Loss(val): {loss:.4f}\t'
